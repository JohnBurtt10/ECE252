...
IMGSTACK* pstack;
int* imageToFetch;
unsigned char* uncompressedIDATs;
unsigned long* idatLength;
size_t* IDAT_sizes_array; // Store the varying IDAT sizes for the images after they've been uncompressed
/* Shared memory ID's */
int shmid_stack;
int shmid_imageToFetch;
int shmid_uncompressedIDATs;
int shmid_idatLength;
int shmid_IDAT_sizes_array;
...
int main(int argc, char* argv[]){
    ...
    *idatLength = 0;
    ...
    shmid_IDAT_sizes_array = shmget(IPC_PRIVATE, sizeof(size_t) * 50, IPC_CREAT | 0666);
...
void detach_and_clean_shm(){
    ...
    shmdt(IDAT_sizes_array);
    shmctl(shmid_IDAT_sizes_array, IPC_RMID, NULL);
}

void decompress_and_push(RECV_BUF *image){
    size_t IDAT_Data_inf_len = 0; /* uncompressed data length */
    ...
    *idatLength += IDAT_Data_inf_len;
    ...
    IDAT_sizes_array[image->seq] = IDAT_Data_inf_len;
...
void create_image(){
    // First, to remove any padded zeros, we must iterate through the sharedstack and copy over the data.
    unsigned char* IDAT_inf = malloc(*idatLength);
    size_t prev = 0;
    for (int i = 0; i < 50; i++){
        memcpy(IDAT_inf + prev, uncompressedIDATs + index_decompressed_idats(i), IDAT_sizes_array[i]);
        prev += IDAT_sizes_array[i];
    }

    // Then, compress the IDATS.
    size_t IDAT_Concat_Data_Def_Len = 0;
    unsigned char *IDAT_Concat_Data_Def = malloc(400*(300 * 4 + 1));
    mem_def(IDAT_Concat_Data_Def, &IDAT_Concat_Data_Def_Len, IDAT_inf, *idatLength, Z_DEFAULT_COMPRESSION);

    // Finally, write the png after compressing idats.
