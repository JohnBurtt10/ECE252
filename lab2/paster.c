...
void curlInit(CURL* curlHandler, char *url, RECV_BUF* pngData);
void createRequest();
...
int main(int argc, char* argv[]){
    ...
    const char* const url[3] = {"http://ece252-1.uwaterloo.ca:2520/image?img=1", 
    ...
    printf("numThreads: %d | imageSegment: %d\n", arguments->numThreads, arguments->imageSegment);
...
void curlInit(CURL* curlHandler, char *url, RECV_BUF* pngData){
    ...
    curl_easy_setopt(curlHandler, CURLOPT_WRITEFUNCTION, write_cb_curl); 
    /* user defined data structure passed to the call back function */
    curl_easy_setopt(curlHandler, CURLOPT_WRITEDATA, (void *)&pngData);

    /* register header call back function to process received header data */
    curl_easy_setopt(curlHandler, CURLOPT_HEADERFUNCTION, header_cb_curl); 
    /* user defined data structure passed to the call back function */
    curl_easy_setopt(curlHandler, CURLOPT_HEADERDATA, (void *)&pngData);
...
void processInput(char *command, char *value, args* destination){
    ...
    if (strcmp(command, THREAD_CMD)){
        destination->numThreads = longToInt;
    }

    if (strcmp(command, RANDOM_PNG_SEG)){
        destination->imageSegment = longToInt;
    }
}

void initalizeArguments(args* argument){
    argument->imageSegment = 1;
